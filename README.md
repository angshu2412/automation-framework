...
├── test-evidence/
│   ├── console-output-1.png        ← Parallel execution logs
│   ├── console-output-2.png        ← Final pass results
│   └── testng-report.png           ← TestNG HTML report
...
### Clone and install
```bash
git clone https://github.com/angshu2412/automation-framework.git
...
# Open in browser instantly (requires Allure CLI)
...
Row position `tr[1]`, `tr[2]` is always stable
regardless of server-assigned IDs.

**JavaScript click for ad-blocked elements** — site
displays fullscreen ad iframes intercepting all
Selenium clicks. JavaScript `arguments[0].click()`
bypasses browser click interception mechanism.

**JavaScript DOM removal for ads** — ad elements
removed from DOM via JavaScript before interactions.
More reliable than locating and clicking a close
button inside a closed Shadow DOM.

**Allure over Extent Reports** — richer reporting
with step-by-step breakdown, timeline view and
failure categorization. Better demonstrates
framework quality to assignment reviewers.

**Checked exceptions wrapped as RuntimeException**
— keeps step definitions and page classes clean.
TestNG handles test failure reporting naturally
without forcing every caller to handle checked
exceptions.
...
1. **Ad interference** — site displays fullscreen
   ...
   Framework handles via JavaScript DOM removal
   before interactions. May still occasionally
   cause flakiness on very slow connections.
...
3. **Parallel thread limit** — running more than 3
   ...
   may cause failures due to server-side rate
   limiting. Keep `thread-count` at 3 or below
   in `testng.xml`.
...
Both scenarios executed successfully in parallel:
- End to end purchase flow with account creation
  and deletion
- Verify first product details match between
  API and UI
...
Both scenarios ran simultaneously on separate
threads (TestNG-PoolService-1 and
TestNG-PoolService-2) demonstrating thread-safe
parallel execution via ThreadLocal WebDriver.

![Console Output 1](test-evidence/console-output-1.png)

### Console Output — Final Results
![Console Output 2](test-evidence/console-output-2.png)

### TestNG HTML Report
![TestNG Report](test-evidence/testng-report.png)
...
**Expected:** A clear message like "No products
found for your search" displayed under
"SEARCHED PRODUCTS"

**Actual:** Section appears completely blank with
no message — user has no feedback that the search
...
**Expected:**
- Card number accepts only 16 numeric digits
- Expiry month accepts only values 01-12
...
in card number and impossible expiry dates

**Impact:** Critical security and data integrity
risk. No payment validation means orders can be
placed without valid payment information. In a
real production system this would cause direct
financial loss and enable fraudulent orders.
...
3. Leave Name, Subject and Message empty
4. Click Submit

**Expected:** Validation errors shown for all
empty required fields before submission is allowed

**Actual:** Form submits successfully with only
email entered — Name, Subject and Message have
no validation

**Impact:** Incomplete contact requests reach
backend causing data quality issues and poor
user experience
...
- Whether touch targets are large enough on
  mobile for comfortable interaction
